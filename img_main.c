...
void switch_image(GtkWidget *image) {
    char filename[10];
    sprintf(filename, "img/%d.jpg", current_image_index);
    gtk_image_set_from_file(GTK_IMAGE(image), filename);
    current_image_index = (current_image_index % size) + 1; // Переход к следующей картинке (здесь 3 - это количество изображений в карусели)
}
...
int main( int argc, char *argv[] ) {
   ...
   g_timeout_add_seconds(3, (GSourceFunc)switch_image, image); // Автоматическая смена картинок каждые 2 секунды   
